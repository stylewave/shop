...
if ($action == 'register') {
	...
	$sql1 = 'SELECT * FROM ' . $ecs->table('company') ;
	$company_list = $db->getAll($sql1);
	...
	if ($_CFG['shop_reg_closed']) {
		...
		$com_code = (isset($_POST['com_code']) ? compile_str(trim($_POST['com_code'])) : '');
		...
		$sql = 'SELECT * FROM ' . $ecs->table('company') . '  WHERE code = \'' .$com_code. '\' ';
...
if ($com_code) {
	if ($db->getOne($sql)) {
				...
				show_message('企业编码不存在，请联系管理员进行添加');
			}
}
		...
		if (register($username, $password, $email, $other, $register_mode) !== false) {
			...
			if ($com_code) {
					$sql = 'UPDATE ' . $ecs->table('users') . ' SET `com_code`=\'' . $com_code . '\' WHERE `user_id`=\'' . $_SESSION['user_id'] . '\'';
				$db->query($sql);
	...
	$com_code=$db->getOne("SELECT com_code  FROM " . $ecs->table("users") . " WHERE user_id = '" .$user_id. "'");
	$companyName=$db->getOne("SELECT companyName  FROM " . $ecs->table("company") . " WHERE code = '" .$com_code. "'");
	...
	$smarty->assign('md5', md5($user_id));
	$smarty->assign('companyName', $companyName);
	$smarty->assign('com_code', $com_code);
