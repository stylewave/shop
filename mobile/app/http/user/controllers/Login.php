...
class Login extends \app\http\base\controllers\Frontend
{
	...
	public function actionRegister()
	{
		if (IS_POST) {
			...
			if (I('enabled_sms') == 1) {
				$com_code = (isset($_POST['com_code']) ? trim($_POST['com_code']) : '');
			...
			if (register($username, $password, $email, $other) !== false) {
				...
				if ($com_code) {
				// $login = $this->db->getOne('UPDATE {pre}users SET com_code=\'' . $com_code . '\''  WHERE `user_id`=\'' . $_SESSION['user_id'] . '\'');
					$sql = 'UPDATE dsc_users SET `com_code`=\'' . $com_code . '\' WHERE `user_id`=\'' . $_SESSION['user_id'] . '\'';
				 $this->db->getOne($sql);
